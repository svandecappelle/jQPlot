...
var labelIdx;if(p.seriesLabelIndex!=null){labelIdx=p.seriesLabelIndex}else{if(this.renderer.constructor===$.jqplot.BarRenderer&&this.barDirection==="horizontal"){labelIdx=0
...
i<this._plotData.length;i++){p._labels.push(this._plotData[i][labelIdx])}}}else{var d=this._plotData;
...
p.setLabels.call(this);for(var i=0;i<p._elems.length;i++){p._elems[i].emptyForce()
...
}var pd=this._plotData;var xax=this._xaxis;var yax=this._yaxis;var elem,helem;for(var i=0,l=p._labels.length;
i<l;i++){var label=p._labels[i];if(p.hideZeros&&parseInt(p._labels[i],10)==0){label=""
}if(label!=null){label=p.formatter(p.formatString,label)}helem=document.createElement("div");
p._elems[i]=$(helem);elem=p._elems[i];elem.addClass("jqplot-point-label jqplot-series-"+this.index+" jqplot-point-"+i);
elem.css("position","absolute");elem.insertAfter(sctx.canvas);if(p.escapeHTML){elem.text(label)
