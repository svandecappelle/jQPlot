...
this.hideZeros=false;this._elems=[];$.extend(true,this,options)};var locationIndicies={nw:0,n:1,ne:2,e:3,se:4,s:5,sw:6,w:7};
var oppositeLocations=["se","s","sw","w","nw","n","ne","e"];$.jqplot.PointLabels.init=function(target,data,seriesDefaults,opts,plot){var options=$.extend(true,{},seriesDefaults,opts);
...
break;default:offset=-elem.outerHeight(true)-this.ypadding;break}return offset};$.jqplot.PointLabels.draw=function(sctx,options,plot){var i,len,p=this.plugins.pointLabels,pd=this._plotData,xax=this._xaxis,yax=this._yaxis,elem,helem;
var that=this;p.setLabels.call(this);for(i=0,len=p._elems.length;i<len;i++){if(p._elems[i]){p._elems[i].emptyForce()
...
}for(i=0,len=p._labels.length;i<len;i++){var label=p._labels[i],location=p.location,ell,elt,elr,elb,et,scl,sct,scr,scb;
if(label==null||(p.hideZeros&&parseInt(label,10)==0)){continue}label=p.formatter(p.formatString,label);
helem=document.createElement("div");p._elems[i]=$(helem);elem=p._elems[i];elem.addClass("jqplot-point-label jqplot-series-"+this.index+" jqplot-point-"+i);
elem.css("position","absolute");if(!$.jqplot.isDarkColor(this.color)){if(p.darkColor!==undefined){elem.css("color",p.darkColor)
}else{elem.addClass("jqplot-point-darkColor")}}else{if(p.brightColor!==undefined){elem.css("color",p.brightColor)
}else{elem.addClass("jqplot-point-brightColor")}}elem.insertAfter(sctx.canvas);if(p.escapeHTML){elem.text(label)
}else{elem.html(label)}if((this.fillToZero&&pd[i][1]<0)||(this.fillToZero&&this._type==="bar"&&this.barDirection==="horizontal"&&pd[i][0]<0)||(this.waterfall&&parseInt(label,10))<0){location=oppositeLocations[locationIndicies[location]]
}ell=xax.u2p(pd[i][0])+p.xOffset(elem,location);elt=yax.u2p(pd[i][1])+p.yOffset(elem,location);
if(this._stack&&!p.stackedValue){var barPoint=that._barPoints[i];if(this.barDirection==="vertical"){elt=(barPoint[0][1]+barPoint[1][1])/2+plot._gridPadding.top-0.5*elem.outerHeight(true);
ell=(barPoint[2][0]+barPoint[0][0])/2+plot._gridPadding.left-0.5*elem.outerWidth(true)
}else{ell=(barPoint[2][0]+barPoint[0][0])/2+plot._gridPadding.left-0.5*elem.outerWidth(true)
}}if(this.renderer.constructor==$.jqplot.BarRenderer){if(this.barDirection=="vertical"){ell+=this._barNudge
}else{elt-=this._barNudge}}elem.css("left",ell);elem.css("top",elt);elr=ell+elem.width();
elb=elt+elem.height();et=p.edgeTolerance;scl=$(sctx.canvas).position().left;sct=$(sctx.canvas).position().top;
scr=sctx.canvas.width+scl;scb=sctx.canvas.height+sct;elem=null;helem=null}}};$.jqplot.postSeriesInitHooks.push($.jqplot.PointLabels.init);
$.jqplot.postDrawSeriesHooks.push($.jqplot.PointLabels.draw)})(jQuery);